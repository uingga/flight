...
> 연결·현재 목록 읽기·정리는 성공했다(상품 조회 0회). 보조 프레임 문제의 실제 원인 확인과
> 신규 전체 실수집은 아직 완료하지 않았다.
...
### 최신-main 복구 통합 — 2026-09-06

아래 실사이트 실행 경로·성과·과거 승인은 원본 브랜치의 인수인계 기록이다. 해당 B PC 원본
staging은 이 Git 브랜치에 없으며 이번 통합에서 독립 검증하지 않았다. 과거의 조회 상한이나
잔여 횟수를 새 실행 승인으로 재사용하지 않는다.

현재 통합본은 신규 staging 수집기·오프라인 테스트만 최신 main에 맞춰 선별한 것이다.
기존 운영 온라인투어 수집기, 어드민, 운영 data/와 스케줄은 변경하지 않았다.

- 마지막 페이지 뒤 정리 중 HTTP 401/403/429를 받은 모의 CDP에서 수정 전 6개 사례 모두
  `review_ready`가 나오는 오류를 재현했다. 수정 후 모두 `failed / access_restriction`이다.
- adapter의 읽기 전용 `failureKind`로 접근 제한/검증 실패 상태를 보존하고, CLI는
  `close()`가 정상 반환해도 정리 **이후** 그 값을 확인한다. `cleanupConfirmed=true`는
  연결 정리 성공일 뿐, 수집 성공이나 운영 준비 완료가 아니다.
- 정리 자체도 실패하면 접근 제한 사유를 우선 보존한다. 이미 검증한 상품은 staging에
  남기되 실패 회차를 성공으로 승격하거나 추가 조회하지 않는다. 다른 CDP 세션·관련 없는
  URL의 응답과 정리 후 이벤트는 이번 회차의 성공/실패 근거에 섞지 않는다.
- 검증 명령: `npm run test:onlinetour-browser`. 실제 캡처 파일이 필요한 선택적 replay는
  원본이 없으면 skip이며, 합성 fixture 통과를 실사이트 성공으로 보고하지 않는다.
- 지역 탐색의 `invalid_paused_request`는 이 종료 오류와 별개다. 보조 프레임을
  무조건 허용하도록 바꾸지 않았다. 전체 도시·월 범위와 전수 조회량은 **미검증**이다.

이하 원본 브랜치의 시점별 기록에서 말하는 검증/승인/실행은 이번 작업의 신규 결과가 아니다.
...
- **원본 전달 당시의 미해결 위험(이번 통합에서 수정):** 정리 중 받은 429가 목록 CLI 최종 성공에 반영되지 않았다. 위 최신-main 복구 기록과 모의 회귀를 참고한다. 지역 CLI의 종료 시 실패 전파와는 별개이며, 수정과 오프라인 테스트 통과를 실사이트 운영 준비 완료로 해석하지 않는다.
