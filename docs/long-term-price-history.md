...
현재는 이 질문에 답할 데이터를 누적하는 단계다. 노선 가격 흐름을 추천 자격이나
TIKIT DROP 선정에 바로 사용하지 않으며, 충분한 관측 기간과 표본을 확인한 뒤 별도로 적용한다.
...
- 가격이 포함된 여행사 ID 대신 생애 기록기와 같은 안정적인 판매 회차 키를 사용한다.
- 검색에 사용한 대표 공항과 실제 운항 공항이 다르면 예약 결과에서 확인한 실제 공항을 기록한다.
