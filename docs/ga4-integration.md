...
아래는 2026-08-31 당시 방식이다. 2026-09-07 항공권별 클릭 통계 개편에서는 노출을 관심 지표에서 제외한다.
...
### 항공권별 클릭 통계 (2026-09-07, 배포 대기)

- 어드민 방문 통계의 기본 화면은 **어떤 항공권을 눌렀나**다. 기본 기간은 최근 7일,
  예약 클릭 횟수순 10개이며 동률이면 상세 조회 횟수, 항공권 ID 순으로 정렬한다.
  `10개 더 보기`로 추가 항공권을 펼치고 기간을 바꾸면 다시 10개로 시작한다.
- 오늘은 오늘 현재까지의 잠정치, 7일·30일은 어제까지의 완결된 날짜를 쓴다.
- `detail_open`, `booking_click`의 `eventCount`만 기록된 `flight_id`별로 합산한다.
  화면 노출, 도시용 동반 이벤트, 제휴용 동반 이벤트를 더하지 않는다. 반복 클릭을 포함한
  횟수이며 예약 완료·구매 수가 아니다. GA4 보고서를 끝까지 조회한 뒤 항공권별로 정렬한다.
- 상세 조회와 예약 클릭은 각각 `30회 · 1명`처럼 횟수와 인원수를 함께 표시한다.
  인원수는 선택 기간 전체의 `eventName` + `flight_id`만으로 별도 조회한 `totalUsers`다.
  가격·노선·일정별 사용자 수를 더하거나 일별 사용자 수를 합산하지 않는다. 두 행동의
  사용자도 서로 겹칠 수 있으며 기기·브라우저가 다르면 같은 사람이 별도로 집계될 수 있다.
  사용자 보고서 실패·표본·데이터 제한·중복 행·횟수 보고서와 집계 시점 불일치에는
  검증된 횟수는 유지하고 `인원 미확인`을 표시한다. 기존 GA4 집계만 조회하며 신규 식별자는 수집하지 않는다.
- 노선·여행사·일정·항공사·가격은 같은 ID에 실제로 기록된 이벤트 값만 표시한다.
  가격이 변한 경우 기록 가격 범위를 표시하며, 상충하는 노선·일정 등은 임의로 선택하지 않는다.
  현재 항공권 캐시를 이용해 과거 정보를 채우거나 다른 ID를 추정해 합치지 않는다.
- ID 없는 행동은 순위에서 제외하고 미수집 건수로 따로 표시한다. 미등록 측정기준,
  조회 실패, 불완전 페이지, GA4 표본·데이터 제한은 0회로 표현하지 않는다.
- 도시별 집계는 보조 보기로 유지한다. 상세 조회·예약 클릭·직접 검색·저장·공유만
  표시하고 노출이나 항공권 재고·노출 대비 비율을 관심 지표로 사용하지 않는다.

**운영 설정 확인:** 2026-09-07 GA4 속성 `524973369`의 맞춤 정의 17개를 읽기 전용으로
확인했다. `route`, `travel_agency`, `departure_date`, `return_date`, `airline`, `price`는
등록되어 있으나 **`flight_id`는 미등록**이다. 배포 시 맞춤 정의에 이름 `항공권 ID`,
범위 `이벤트`, 이벤트 매개변수 `flight_id`를 등록해야 한다. 이번 구현에서는 설정을
변경하지 않았다. 등록 전 과거 기록은 소급 추정하지 않으며 새 기록의 처리에는 시간이 필요하다.

검증: `npm run test:flight-interest`, `npm run test:flight-interest-ui`, `npm run build` 통과.
UI 테스트는 기본 10개·더 보기·기간 전환·빈 기록·조회 실패와 390px/320px 화면을 확인한다.
로컬 API 인증 거부(401), GA4 설정 없음 안내(200), 공개 호스트의 미리보기 차단(404)도 확인했다.
운영 GA4 Data API 실조회는 로컬에 서비스 계정 설정이 없어 검증하지 못했다. 전용 로컬 서버의
`/preview/flight-interest`는 예시 데이터임을 명시하며 운영 환경에서는 404다.
`?state=empty`, `?state=unavailable`로 빈 기록과 조회 실패를 검토할 수 있다.
`?state=users-unavailable`은 횟수는 확인되지만 인원수는 확인하지 못한 경우다.
