...
이미 등록되어 있었다. 사용자의 진행 승인 뒤 2026-09-07 맞춤 정의에 이름 `항공권 ID`,
범위 `이벤트`, 이벤트 매개변수 `flight_id`를 등록하고 총 18개 목록에서 저장을 확인했다.
등록 전 과거 기록은 소급 추정하지 않으며 새 기록의 처리에는 시간이 필요하다.
