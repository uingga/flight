...
- TE31 짧은 링크는 `tikitikit_te31_{공유그룹코드}`를 써서 게시글별 성과가 섞이지 않게 한다.
- `utm_content`: `top_link`, `flight_1`, `bottom_link` 등 링크 위치. 지금부터 기록하되 화면에는 아직 노출하지 않는다.
- 어드민에는 네이버 블로그와 TE31을 함께 묶어 유입자, 참여 유입률, 상세 열람률, 예약 이동률, 실제 상세로 본 도시를 핵심 지표로 보여준다.
- 홍보 채널 성과는 오늘을 포함해 조회하고, 오늘 수치는 GA4 처리가 끝나기 전까지 잠정치로 표시한다.
