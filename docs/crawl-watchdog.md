...
1. Supabase Cron이 5분마다 `GET /api/internal/crawl-watchdog`를 호출한다.
2. API는 `data/all-flights-cache.json`의 `fullCrawlUpdatedAt`으로 마지막 전체 수집 완료
   시각을 확인한다. 부분 여행사 복구는 이 값을 바꾸지 않는다.
3. 예정 회차가 5분 이상 늦었고 실행 중이거나 최근 요청된 보조 실행이 없을 때만
   `daily-crawl.yml`을 `trigger_source=watchdog`으로 실행한다.
4. 같은 cron 회차의 성공한 예약 실행이 이미 있으면 배포 전파 중 캐시가 잠시 오래되어도
   보조 실행을 만들지 않는다.
5. 기존 preflight와 concurrency가 뒤늦은 예약 실행 및 중복 크롤을 한 번 더 막는다.

GitHub의 `Crawl Schedule Watchdog` 자체 예약 cron은 사용하지 않는다. 정시 확인은
Supabase Cron이 맡고, GitHub 워크플로는 마이리얼트립 완료 이벤트와 수동 점검만 보조로 남긴다.
