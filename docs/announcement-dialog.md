...
현재 공지는 `모두투어 점검 안내`이며 키는 `tikitikit-service-update-20260919-modetour-maintenance-v1`입니다. 종료 시각이 확인되지 않아 `endsAt: null`로 표시를 유지합니다. 점검 종료 확인 후 명시적으로 공지를 내립니다. 이전 유류할증료 공지의 비노출 선택은 이 새 공지에 적용하지 않습니다. 수집 일정·접근 제한 판단·예약 링크는 변경하지 않습니다.
