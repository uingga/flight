...
현재 공지는 `모두투어 점검 안내`이며 키는 `tikitikit-service-update-20260919-modetour-maintenance-v1`입니다. 사용자 요청에 따라 2026-09-19 06:00 KST부터 자동 비노출됩니다. 이는 공지 노출 마감이며 여행사의 점검 완료 예정 시각을 의미하지 않습니다. 이전 유류할증료 공지의 비노출 선택은 이 새 공지에 적용하지 않습니다. 수집 일정·접근 제한 판단·예약 링크는 변경하지 않습니다.
