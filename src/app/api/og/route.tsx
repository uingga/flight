...
import { BlogGroupOgCard } from './BlogGroupOgCard';
...
export async function GET(request: NextRequest) {
    ...
    return new ImageResponse(
        (
            blog && searchParams.get('brand') === '1' ? <BlogBrandOgCard /> : blog && group?.title ? <BlogGroupOgCard cities={group.arrival.split('·').map(city => city.trim())} /> : group?.title ? <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', background: '#ff385c', color: 'white', padding: '70px', fontFamily: 'Pretendard' }}>
