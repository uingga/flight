...
export async function createOnlineTourBrowserAdapter(client: CdpClient) {
    ...
    return { inspect, readPage, close, diagnostics, authorizeProductRequests,
        // close() confirms transport cleanup, not collection success. Preserve restrictions
        // observed after the last page and during stop/drain/detach for the caller to check.
        get failureKind(): ListReadError['kind'] | null {
            return accessLatched ? 'access' : validationLatched ? 'validation' : null;
        },
