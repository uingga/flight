...
import { selectMyrealtripNaverComparison } from '@/lib/myrealtrip-naver-verification';
...
export async function createPublicFlightsResponse(searchParams: URLSearchParams) {
    ...
    try {
        ...
        let loadedNaverPrices: unknown = null;
        try {
        ...
        // MRT and Trip.com wait for a fresh exact-itinerary comparison. Missing
        // or unreadable comparison data must not make either source public.
        const beforeMrtGate = allFlights.filter(flight => flight.source === 'myrealtrip').length;
        const beforeTripcomGate = allFlights.filter(flight => flight.source === 'tripcom').length;
        allFlights = filterNaverVerifiedOffers(allFlights, loadedNaverPrices);
        filterSummary.reasons.naverMrtGate = beforeMrtGate
            - allFlights.filter(flight => flight.source === 'myrealtrip').length;
        filterSummary.reasons.naverTripcomGate = beforeTripcomGate
            - allFlights.filter(flight => flight.source === 'tripcom').length;

        // Deduplicate only eligible offers: an unverified cheap fare must not
        // suppress another agency's displayable offer for the same itinerary.
