...
/** @param {{flight: any, buildPick: Function, resolveSelection?: Function, env?: any, invoke?: Function}} options */
export async function publishAdminPick({flight,buildPick,resolveSelection,env=process.env,invoke=undefined}) {
 ...
 const current=resolveSelection ? resolveSelection(snapshot.cache.flights)[0] : snapshot.cache.flights.find(item=>item.id===flight.id);
 if(!current)throw Error('selected flight no longer available');
 const pick=buildPick(snapshot.pick,current);
 if(snapshot.pick?.date===pick.date&&snapshot.pick?.flightId===pick.flightId&&JSON.stringify(snapshot.pick?.selectedFlightKeys||null)===JSON.stringify(pick.selectedFlightKeys||null))return {pick,commitSha:null,alreadySelected:true};
