...
export async function scrapeTtangDiscount(): Promise<Flight[]> {
    ...
    try {
        const url = 'https://mm.ttang.com/ttangair/search/discount/today.do?trip=RT&gubun=T';
        await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 30000 });
        await page.waitForSelector('li.exair1', { timeout: 10000 }).catch(() => { });
        await randomDelay(1, 2);

        // li.exair1 에서 데이터 추출 (프로모션과 동일 구조)
        const items = await page.evaluate(() => {
            const lis = document.querySelectorAll('li.exair1');
            return Array.from(lis).map(li => {
                ...
                return {
                    ...
                    arrCity: el.dataset.arrcitydesc || '',
                    arrAirport: el.dataset.arrcitycode || '',
                    fromDate: el.dataset.fromsupplydate || '',
                    toDate: el.dataset.tosupplydate || '',
                    price: parseInt(el.dataset.totalprice || '0') || 0,
                    masterId: el.dataset.masterid || '',
                    minimumcnt: el.dataset.minimumcnt || '',
                };
            });
        });
        ...
        for (const f of items) {
            if (!f.fromDate || !f.toDate || f.price <= 0) continue;
            if (f.minimumcnt && parseInt(f.minimumcnt) > 1) continue;

            const depInfo = DEP_CITY_MAP[f.depAirport] || { city: f.depCity || '서울', airport: f.depAirport || 'ICN' };

            const fromDate = parseYYYYMMDD(f.fromDate.replace(/-/g, ''));
            const toDate = parseYYYYMMDD(f.toDate.replace(/-/g, ''));
            ...
            while (currentDate <= effectiveToDate) {
                if (currentDate >= today) {
                    ...
                    const key = `${f.airline}|${depDateStr}|${f.price}|${f.depCity}|${f.arrCity}|disc`;
                    if (!processedKeys.has(key)) {
                        ...
                        const textFragment = f.arrCity
                            ? `#:~:text=${encodeURIComponent(f.arrCity)}`
                            : '';
                        const link = `https://mm.ttang.com/ttangair/search/discount/today.do?trip=RT&gubun=T${textFragment}`;

                        const flight: Flight = {
                            id: `ttang-disc-${f.masterId}-${depDateStr}`,
                            source: 'ttang',
                            airline: f.airline || '알 수 없음',
                            departure: {
                                city: depInfo.city,
                            ...
                            currency: 'KRW',
                            link,
                            searchLink: 'https://mm.ttang.com/ttangair/search/discount/today.do?trip=RT&gubun=T',
                            region: getRegionByCity(f.arrCity) || '',
                        };
