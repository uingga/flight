...
function MetricValue({ cell, delta }: { cell?: Metric; delta: number | null }) {
    return <>{cell ? <><strong>{cell.value.toLocaleString()}</strong><span>{delta === null ? '전날 비교 없음' : `전날 대비 ${delta > 0 ? '+' : ''}${delta.toLocaleString()}`}</span><span>{cell.day} 기준</span></> : <><strong>—</strong><span>확인값 없음</span></>}</>;
}
export function PromotionDailyView({ data }: { data: DailyHistory }) {
    const today = kstDay();
    const [selectedSource, setSelectedSource] = useState('threads');
    const activeSource = data.latest.some(row => row.source === selectedSource) ? selectedSource : data.latest[0]?.source;
    return <div className={styles.panel}>
        <div className={styles.tabs} aria-label="성과 채널 선택">{data.latest.map(({ source, payload }) => <button key={source} type="button" aria-pressed={activeSource === source} onClick={() => setSelectedSource(source)}>{names[source] || source}<span>{payload.posts.length}</span></button>)}</div>
        <div className={styles.toolbar}><span>글별 성과 · {activeSource === 'ga4' ? '날짜별 사이트 행동' : '외부 누적 반응'}</span><span>매일 오전 9시 수집 요청</span></div>
        {!data.latest.length && <p role="status">아직 저장된 일별 성과가 없습니다.</p>}
        {data.latest.filter(row => row.source === activeSource).map(({ source, payload }) => {
            ...
            const primaryKeys = source === 'ga4' ? ['users', 'detailUsers', 'bookingUsers'] : keys.slice(0, 3);
            return <section key={source} className={styles.source} aria-label={`${names[source] || source} 일별 성과`}>
                <div className={styles.sourceHeading}><h3>{names[source] || source}</h3><span>{outcome[payload.outcome]}{stale ? ' · 최신 확인 필요' : ''}</span></div>
                <details className={styles.collectionInfo}><summary>수집 상태 · 확인 기준</summary>
                ...
                {!payload.posts.length && <p className={styles.empty}>저장된 글이 없습니다.</p>}
                <div className={styles.posts}>{payload.posts.map(post => <article key={post.id} className={styles.post}>
                    ...
                    <details className={styles.postContent}>
                        <summary>
                            <span className={styles.postExcerpt}>{post.title || post.id}</span>
                            <span className={styles.showContent}>본문 펼치기 +</span>
                            <span className={styles.hideContent}>본문 접기 −</span>
                        </summary>
                        {post.url && <a href={post.url} target="_blank" rel="noopener noreferrer">원문 보기 ↗</a>}
                    </details>
                    ...
                    <dl className={styles.metrics}>{primaryKeys.map(key => <div key={key}>
                    ...
                    <details className={styles.metricDetails}><summary>세부 지표 · 확인 시각</summary><dl className={styles.metrics}>{keys.map(key => <div key={key}><dt>{METRIC_LABELS[key] || key}</dt><dd><MetricValue cell={post.metrics[key]} delta={post.metrics[key] ? dailyDelta(post.metrics[key], post.id, key, source, data.sources) : null} />{post.metrics[key] && <time dateTime={post.metrics[key].observedAt}>확인 {time(post.metrics[key].observedAt)}</time>}</dd></div>)}</dl></details>
                </article>)}</div>
            </section>;
        })}
        <details className={styles.collectionInfo}><summary>최근 실행 기록 · {data.runs.length}일</summary><ul className={styles.history}>{data.runs.map(run => <li key={run.day}>
        ...
        <details className={styles.collectionInfo}><summary>통계 읽는 방법</summary><p>외부 반응은 누적 관측값, 사이트 행동은 날짜별 잠정치입니다. GA4는 한국 시간 기준 어제부터 3일 전까지 다시 확인합니다.</p><p>—는 확인 불가입니다. 첫날이나 바로 전날 기록이 없으면 차이를 계산하지 않습니다. 실패한 값은 마지막 확인값과 시각을 유지합니다.</p><p>같은 사람이 여러 글에 포함될 수 있어 글별 인원을 합산하지 않습니다. 예약 이동은 여행사 페이지로 나간 행동이며 구매·발권 완료가 아닙니다.</p></details>
    </div>;
}
