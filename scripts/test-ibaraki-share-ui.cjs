...
(async () => {
  ...
  try {
    for (const width of [390, 320, 1440]) {
      ...
      assert.equal(new URL(page.url()).searchParams.get('flight'), ids[1]);
      assert.equal(await cards.count(), 3, 'three cards stay behind the detail');
      await page.goBack();
      ...
      console.log(`PASS ${width}px: three schedules, selected detail, back to list, browse more, attribution`);
      await page.close();
    }
