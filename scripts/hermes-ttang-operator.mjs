...
function run(mode) {
    ...
    if (allDetails && mode !== 'pilot') throw new Error('--all-details is pilot-only');
    ...
    let result;
    try {
        result = spawnSync(process.execPath, [script, ...(allDetails ? ['--all-details'] : [])], {
            cwd: ROOT,
            stdio: 'inherit',
        });
    } finally {
    ...
    const childExitCode = result?.status ?? 1;
    const exitCode = childExitCode === 0 && !producedNewSummary ? 1 : childExitCode;
    const payload = {
        action: mode,
