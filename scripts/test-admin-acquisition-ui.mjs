...
try {
 ...
 assert.equal(await source.locator('table').first().locator('tbody tr').count(),10);
 ...
 await source.getByRole('combobox').selectOption('전체');assert.equal(await source.locator('table').first().locator('tbody tr').count(),10);
