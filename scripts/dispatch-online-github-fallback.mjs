...
async function main() {
    ...
    assertOnlineGithubFallback({cache,expectedAt:policy.onlineExpectedAt});
    ...
    const marker=path.join(state,createHash('sha256').update(policy.onlineExpectedAt).digest('hex')+'.json');
    if(fs.existsSync(marker)) {console.log('OnlineTour dispatch already attempted for this slot; not repeated.');return;}
    fs.writeFileSync(marker,JSON.stringify({expectedAt:policy.onlineExpectedAt,status:'attempted'}),{flag:'wx'});
    ...
    const response=await fetch(`https://api.github.com/repos/${match[1]}/${match[2]}/actions/workflows/onlinetour-fallback.yml/dispatches`,{
        method:'POST',headers:{Authorization:`Bearer ${token}`,Accept:'application/vnd.github+json','Content-Type':'application/json',
            'X-GitHub-Api-Version':'2022-11-28'},body:JSON.stringify({ref:'main',inputs:{expected_at:policy.onlineExpectedAt}}),
        signal:AbortSignal.timeout(20000)});
    fs.writeFileSync(marker,JSON.stringify({expectedAt:policy.onlineExpectedAt,status:response.status===204?'dispatched':'failed',httpStatus:response.status}));
    if(response.status!==204)throw Error('github_dispatch_failed');
    console.log('OnlineTour GitHub fallback dispatched for '+policy.onlineExpectedAt);
}
