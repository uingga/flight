...
const verified = load('src/lib/threads-post-links.ts', { './threads-tracking': tracking, './threads-post-links.json': { default: require('../src/lib/threads-post-links.json') } });
const posts = verified.connectVerifiedPostLinks([
    ...
    {id: 'custom', permalink:'', trackingContent:'tae_dyg_20260916',shareCode:null},
...
const api = load('src/app/api/threads-insights/route.ts', {
    ...
    '@/lib/ga4': {
        ...
        runReport: async (_config, request) => {
            ...
            assert.ok(JSON.stringify(request.dimensionFilter).includes('sessionSource'));
            assert.ok(!JSON.stringify(request.dimensionFilter).includes('share_'));
            ...
            return { rows: event ? [row(['share_xmodetour-CHI-20107439', 'detail_open'], [1, 1])]
                : [row(['share_xmodetour-CHI-20107439'], [1, 1]), row(['share_group_pqc1438'], [2, 2]), row(['tae_dyg_20260916'],[2,1])] };
        },
    },
}, { Date: FakeDate });
...
(async () => {
    ...
    assert.equal(first.body.posts[2].attribution.users, 2);
    assert.equal(first.body.posts[2].attribution.bookingUsers, 0);
    assert.equal(first.body.posts[1].attribution.sessions,2);
    assert.equal(first.body.posts[1].attribution.users,1);
    clock += 9 * 60_000;
