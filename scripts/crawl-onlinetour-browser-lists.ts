...
export interface ListAdapter {
    ...
    readonly failureKind?: ListReadError['kind'] | null;
...
export async function executeBrowserLists(adapter: ListAdapter, repositoryRoot: string, scopes: ListScope[],
    ...
    try {
        ...
        // Re-read the latch AFTER cleanup, even when close resolved normally.
        // An access restriction remains the primary failure if cleanup also failed.
        if (adapter.failureKind === 'access') failure = 'access_restriction';
        else if (adapter.failureKind) failure = failure || 'browser_adapter_failed';
    }
...
async function main(): Promise<void> {
    ...
    if (args.mode === 'inspect') {
        ...
        console.log(JSON.stringify({ mode: 'inspect', readOnly: true, productionReady: false, snapshot,
            failureKind: adapter.failureKind, diagnostics: adapter.diagnostics }));
        process.exitCode = snapshot.restricted || adapter.failureKind ? 1 : 0;
        return;
    }
