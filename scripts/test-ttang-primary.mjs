...
test('one-month KST plan clamps month end; no 60-day expansion',()=>{
    ...
    const b=ttangDatePlan(new Date('2027-01-30T15:00:00Z'));
    assert.equal(b[0],'20270131');assert.equal(b.at(-1),'20270228');assert.equal(b.length,29);
});
function fixture(){
    const startedAt='2026-09-07T09:00:00Z',completedAt='2026-09-07T09:05:00Z';
...
test('shuffled dates validate as a complete permutation; omitted/duplicate dates or a changed seed fail', () => {
    ...
    const now=Date.parse('2026-09-07T09:06:00Z');
...
test('no-new-detail round is valid; old timestamps never count as new verification',()=>{
    assert.equal(validateTtangEvidence(fixture(),'test',Date.parse('2026-09-07T09:06:00Z')).timeVerified,0);
});
...
}))test('rejects '+name,()=>{const b=fixture();change(b);assert.throws(()=>validateTtangEvidence(b,'test',Date.parse('2026-09-07T09:06:00Z')));});
