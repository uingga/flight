...
if (process.argv.includes('--fallback')) throw new Error('pilot cannot be combined with --fallback');
await ensureTtangDebugChrome({ profileDir, port });

const result = spawnSync(
    process.execPath,
    [path.join(ROOT, 'scripts', 'run-ttang-browser-staging.mjs'), `--cdp=http://127.0.0.1:${port}`, ...(allDetails ? ['--all-details'] : [])],
