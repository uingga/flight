...
test('GitHub/C competing admission gives only one collector',async()=>{const {api}=fixture();const r=await Promise.all(['github','C'].map(host=>acquireMrt(api,{slot,host,sha})));assert.equal(r.filter(Boolean).length,1);});
