...
test('pre-rollout completed scheduled run prevents re-collection', async () => {
    const runs = [{ id: 9, event: 'schedule', status: 'completed', conclusion: 'success', created_at: '2026-09-08T07:00:00Z', display_title: 'MyRealTrip · 40 3 * * *' }];
...
test('previous day success does not suppress today', async () => {
    const runs = [{ event: 'schedule', status: 'completed', conclusion: 'success', created_at: '2026-09-07T07:00:00Z', display_title: '40 3 * * *' }];
...
test('dispatch carries exact slot and durable ref is deterministic', async () => {
    ...
    assert.equal(mrtClaimRef(slot.expectedAt), 'tags/mrt-slot/20260908T034000000Z');
});
