...
async function main() {
    ...
    let backend: Awaited<ReturnType<typeof createLiveCatalogueBackend>> | undefined;
    try {
        check();
        ...
        const slot=mode==='--manual-once'?'manual-once-'+new Date(now+9*3600000).toISOString().slice(0,10):continuation?'validation-continue-30-execution-'+CONTINUATION_PARENT:mode==='--validate-status-fix'
        ...
        const ownedTab=mode==='--scheduled' || mode==='--manual-once';
        backend=await createLiveCatalogueBackend(ownedTab);
        browserStarted=true;
        let snapshot: {region:string;restricted:boolean;emptyInventoryVerified?:boolean}={region:'AS',restricted:false};
        if(!ownedTab) {
        ...
        if(backend.close) { cleanupConfirmed=false; await backend.close(); cleanupConfirmed=summary.cleanupConfirmed===true; }
        ...
        if(backend?.close) {
            try { await backend.close(); cleanupConfirmed=true; } catch { cleanupConfirmed=false; }
        }
        ...
        fs.writeFileSync(path.join(state,'failure-'+request.id+'.json'),JSON.stringify({id:request.id,at:new Date().toISOString(),reason,restricted,cleanupConfirmed,runId:summary?.runId || null}));
