...
require('tsx/cjs');
const { connectDedicatedChrome, createOnlineTourBrowserAdapter } = require('../src/lib/onlinetour-browser-adapter.ts');
const { ListReadError } = require('../src/lib/onlinetour-list-traversal.ts');
const knownReasons = new Set([
    'dedicated_chrome_discovery_failed', 'cdp_connect_deadline', 'cdp_connect_failed',
    'cdp_disconnected', 'cdp_command_deadline', 'cdp_command_failed', 'cdp_send_failed',
    'require_exactly_one_existing_list_tab',
...
if (process.argv.length !== 3 || process.argv[2] !== '--consent-confirmed') {
    ...
    try {
        emit('connecting_dedicated_chrome', { readOnly: true, browserMode: 'persistent_dedicated_loopback' });
