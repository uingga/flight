...
{
    const failed = event('2026-09-03T06:36:00', { preserved: true, value: 1261 });
    const pc = event('2026-09-03T06:41:00', { localFallback: true, value: 1295 });
    assert.equal(pickFinalEvent([failed, pc]), pc);
    const skip = event('2026-09-03T13:42:00', { skipped: true, skipReason: 'circuit' });
...
{
    const now = kst('2026-09-04T12:00:00'); // 10:12 회차 진행 중
    const events = [
        event('2026-09-03T06:36:00', { preserved: true, value: 1261 }),
        event('2026-09-03T06:41:00', { localFallback: true, value: 1295 }),
        event('2026-09-03T10:33:00', { skipped: true, skipReason: 'schedule', value: 0 }),
        event('2026-09-03T13:42:00', { skipped: true, preserved: true, skipReason: 'circuit', value: 0 }),
        event('2026-09-03T13:48:00', { localFallback: true, value: 1282 }),
        event('2026-09-03T16:49:00', { skipped: true, skipReason: 'schedule', value: 0 }),
    ...
    assert.equal(at('2026-09-03T06:17:00').status, 'pc');
    assert.equal(at('2026-09-03T06:17:00').value, 1295);
    assert.equal(at('2026-09-03T06:17:00').events.length, 2, '자동 실패 + PC 성공이 한 칸에 모인다');
    assert.equal(at('2026-09-03T10:12:00').status, 'unscheduled');
    assert.equal(at('2026-09-03T13:23:00').status, 'pc');
    assert.equal(at('2026-09-03T13:23:00').value, 1282);
    assert.equal(at('2026-09-03T16:31:00').status, 'unscheduled');
    assert.equal(at('2026-09-04T06:17:00').status, 'skipped');
    assert.equal(at('2026-09-04T06:17:00').final?.skipReason, 'circuit');
    // 최근 회차 10:12는 땡처리 예정 없음 → 대기 아님
    const latest = bars[bars.length - 1];
    assert.equal(latest.isLatest, true);
    assert.equal(fmt(latest.slotAt), fmt(new Date(kst('2026-09-04T10:12:00')).toISOString()));
    assert.equal(latest.status, 'unscheduled');
    // 09-02 이전 칸은 기록 없음
    assert.equal(at('2026-09-02T13:23:00').status, 'missing');
}

// 일반 여행사: 진행 중 회차는 'pending', 전체 수집이 끝났는데 기록 없으면 'missing'
{
    const now = kst('2026-09-04T10:20:00');
    const events = [event('2026-09-04T06:35:00', { value: 313 })];
    ...
    const done = buildSourceSlotBars({ source: 'ybtour', events, now, completedThrough: kst('2026-09-04T10:18:00') });
...
// 수동 캡처는 회차 밖 시각이어도 가장 가까운 이전 회차 칸에 막대로 들어간다
{
    ...
    const events = [
        ...
        event('2026-09-01T12:31:00', { manual: true, value: 42 }),
        event('2026-09-01T14:52:00', { preserved: true, value: 0 }),
        event('2026-09-01T15:47:00', { manual: true, value: 55 }),
        event('2026-09-01T16:49:00', { preserved: true, value: 0 }),
    ];
    ...
    assert.equal(last[0].events.length, 2, '10:12 칸에 자동 성공 + 수동 캡처가 모인다');
}

// 명시적인 테스트 픽스처: 실행 상태, 경계 시각, 미래 기록 제외
{
    const now = kst('2026-09-05T13:24:00');
    const currentRun = { startedAt: '2026-09-05T13:23:30+09:00', status: 'in_progress', stage: 'crawling' as const, plannedSources: ['ybtour'], skippedSources: [] };
    ...
    assert.equal(after.at(-1), kst('2026-09-05T13:23:00'));
    const future = buildSourceSlotBars({ source: 'ybtour', now, events: [event('2026-09-05T16:32:00')] });
