...
- 오늘의 표는 08:17 일반 여행사 크롤이 운영 API에 반영된 직후 아침 1회, PC 네이버 필터가
  성공해 운영 API에 반영된 직후 오후 1회 새로 선정한다. 다른 일반 여행사 크롤 회차는 현재
  표가 사라졌거나 유효하지 않을 때만 `select-today-pick.mjs --repair`로 복구한다.
