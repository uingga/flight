...
| `modetour` | 모두투어 | API (GitHub) / manual screenshot fallback | `src/lib/scrapers/modetour.ts`, `src/lib/scrapers/modetour-manual.ts` |
...
2. **가격·시간 갱신 (자동, 하루 2회 KST 05:00/12:05)**: `.github/workflows/myrealtrip-scrape.yml` →
   ...
   완료된 목록을 같은 회차의 추가 날짜 후보로 사용한다. 이 목록은 월별 기준가에 합치거나 파일에
   저장하지 않으며 과거 목록을 재사용하지 않는다. 서울 출발 왕복 후보 중 기존 마이리얼트립 날짜와
   겹치지 않는 최대 8개만 추가로 확인하고, 실제 예약 화면에서 직항·출도착 공항·가격·시간을 모두
   확인한 경우만 마이리얼트립 항공권으로 노출한다. 인터파크 목록 수집이 실패하면 추가 후보만
   건너뛰고 정규 마이리얼트립 수집은 계속하며, 보조 후보의 개별 실패도 정규 수집 실패율에 넣지 않는다.
   네이버 운영 수집은 호출하지 않는다. 워커 1개로 직렬 실행하고 노선 사이 4~8초, 10건마다
   30~60초를 쉰다. 8건 연속 빈 운임 또는 20건 표본 성공률 20% 미만이면 즉시 중단하고
   기존 캐시를 유지한 채 24시간 마이리얼트립 회로를 저장한다. 정규·수동 회차 모두 성공 시각과
   관계없이 실행하되 동시 워크플로는 금지한다. 열려 있는 차단 회로도 그대로 적용한다.
...
- 롯데관광의 소량 목록은 예외다. 직전 원본 30건 미만이며 기존 항공권이 모두 출발한 뒤 정상 응답이 0건이면 재고 소진으로 채택한다. 아직 출발하지 않은 표가 사라졌다면 이전 표를 보존하고 다음 정규 회차에서 재확인하되, 수량만으로 접근 차단 회로를 열지 않는다. 응답 오류·명시적 차단·대량 급감 보호는 유지한다.
- 땡처리닷컴 B·C PC 전용 주 수집은 31일 날짜별 목록·요청 횟수·응답 형식의 완료 증거가
  독립 검증될 때만 위 60% 급감 규칙의 예외로 다룬다. 실제 재고 감소를 차단으로 오인하지 않기
  위한 예외이며, 0건·401·403·429·CAPTCHA·날짜 누락·응답 붕괴는 기존처럼 보존·중단한다.
  상세 판단과 복구 근거는 `docs/ttang-count-recovery-20260924.md`를 따른다.
...
- 차단된 일반 여행사는 Windows PC가 해당 소스만 대체 수집한다. PC에서도 차단되면 그 소스의
  PC 대체 수집을 24시간 중단한다. 단, 모두투어는 GitHub 실패 뒤 PC 자동 접속을 하지 않는다.
  운영자가 일반 Chrome 결과 화면을 캡처해 전달하면 검증된 카드만 기존 캐시에 부분 병합하고,
  애매한 카드는 확인 전까지 반영하지 않는다. 상세 기준은 `docs/travel-agency-crawl-safety.md`를 따른다.
- 모두투어 중국(`CHI`)은 `전체` 조회를 사용하지 않는다. 화면에 노출되는 10개 도시를
  `ArrivalCity`별로 개별 조회해 합치며, 기존 데이터가 있던 도시 하나라도 실패하면 불완전
  결과로 처리해 이전 모두투어 데이터를 보존한다.
- 땡처리닷컴 시간 보강은 최종 노출 필터 뒤에 신규 미조회 항공권부터 회차당 최대 20개 노선만
...
- 인터파크 월별 기준가는 `출발 권역|도착 도시` 조합으로 수집한다. 인천·김포는 `SEL`로 묶고
  ...
  빠른 출발 인기 도시 묶음은 월별 기준가와 섞지 않고, 하루 2회 마이리얼트립 정규 회차 직전에
  새로 받아 서울 출발 보조 후보로만 사용한다.
...
- 시스템성 일시 오류는 진행 가격과 위치를 저장하고 10~20분 한 번 쉰 뒤 실패 항공권 다음부터
  같은 프로세스에서 이어간다. 휴식 전후를 합쳐 호스트별·일일 페이지 이동 상한을 유지한다. 재개 뒤에도
  조기 종료되면 성공한 부분 가격은 운영 데이터에 반영하지만 그 불완전 회차로 오늘의 표는 선정하지
  않는다. 성공 0건이나 명시적 차단 회차의 부분 가격은 반영하지 않는다.
- 브라우저 실행·대기열 준비처럼 실행 상태 파일상 네이버 요청이 0건인 단계에서 실패한 경우에만
  15~30분 뒤 전체 프로세스를 한 번 재시도한다. 요청이 시작됐거나 상태 파일이 없으면 재시도하지 않는다.
- 네이버 페이지 이동은 대조 노선을 포함해 A 최대 200회, C 최대 250회, 같은 KST 날짜
  합계 최대 450회다. 실패 키는 당일 재시도하지 않는다. 성공 키 중 마이리얼트립·트립닷컴의
  노출 가능 표만 새 여행사 가격 확인 뒤 또는 네이버 확인 6시간 뒤 정규 회차에서 당일 1회
  추가 확인할 수 있다. 다른 여행사의 성공 키는 당일 재조회하지 않는다. 변경 없는 성공 키의
  정기 후보 간격은 마이리얼트립·트립닷컴 1 KST 날짜, 다른 여행사 2 KST 날짜다. 실제 요청은
  남은 일일 예산·정규 회차·접근 제한 회로가 허용할 때만 실행한다. 브라우저 실행에는
