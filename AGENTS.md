...
- GitHub `naver-crawl.yml`은 운영 데이터를 쓰지 않는 수동 진단 전용이다. `myrealtrip` 최대 3건만
  ...
  워크플로도 이를 호출하지 않는다.
- Windows `TikitikitNaverCrawl`은 11:12/14:23/17:31 KST에 기동한다. 11:12 일반 크롤이
  반영되면 정상 수집된 여행사와 당시 최신 마이리얼트립만 먼저 네이버에서 확인한다. 이전 캐시를
  보존한 여행사는 14:23 일반 크롤까지 기다렸다가 실제로 복구된 소스만 2차 확인한다. 17:31은
  14:23 작업 자체가 기동되지 못했을 때의 시작 안전망이며 20:30 폴백은 사용하지 않는다.
  두 단계는 처리 키와 페이지 이동량을 공유해 KST 날짜당 합계 200회를 넘지 않는다. 1차에서
  403·429·CAPTCHA가 감지되거나 시스템 오류로 조기 종료되면 같은 날 2차를 실행하지 않는다.
  오늘의 표는 모든 단계가 끝난 뒤 그날 정상 수집된 여행사만 후보로 삼아 한 번 선정한다.
...
- 시스템성 일시 오류는 진행 가격과 위치를 저장하고 10~20분 한 번 쉰 뒤 실패 항공권 다음부터
  같은 프로세스에서 이어간다. 휴식 전후를 합쳐 페이지 이동 200회 상한을 유지한다. 재개 뒤에도
...
- 네이버 페이지 이동은 대조 노선을 포함해 하루 최대 200회다. 성공·실패 키는 같은 KST 날짜에
  다시 조회하지 않으며 다음 KST 날짜부터 다시 후보가 될 수 있다. 변경 없는 성공 키는 통상
  2일 간격으로 확인한다. 실제 브라우저 실행에는
