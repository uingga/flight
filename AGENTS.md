...
2. **가격·시간 갱신 (자동, 하루 3회 KST 07:05/16:03/19:59)**: `.github/workflows/myrealtrip-scrape.yml` →
   ...
   성공 후 6시간 안의 임의 중복 실행은 건너뛴다. 단, 19:59 예약 회차는 세 번째 정규 수집을
   위해 이 중복 방지만 해제한다. 열려 있는 차단 회로와 동시 실행 금지는 그대로 적용한다.
...
- Windows `TikitikitBlockedSourceCrawl`은 일반 크롤과 같은 08:17/11:12/14:23/17:31 KST에
  ...
  자동 대상에서 제외하며 어드민에 수동 캡처 필요 상태만 표시한다. 땡처리닷컴은 GitHub 차단
  회로가 열린 동안 08:17·14:23에만 PC 대체 수집하고 11:12·17:31에는 접속하지 않는다.
